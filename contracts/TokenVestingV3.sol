 ...
 * @title TokenVesting
 */
contract TokenVestingV3 is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	struct VestingSchedule {
		...
		uint32 start;
		// whether or not the vesting is paused
		// putting this into the end (empty space) of storage SLOT 1
		bool paused;
		// SLOT 2
	...
	event Paused(address _by, address indexed _beneficiary, bytes32 indexed _vestingScheduleId, bool indexed _paused);
	...
	*/
	...
	/**
	 * @dev Reverts if the vesting schedule does not exist or has been revoked or is paused.
	 */
	modifier onlyIfVestingScheduleNotPaused(bytes32 vestingScheduleId) {
		require(vestingSchedules[vestingScheduleId].initialized == true);
		require(vestingSchedules[vestingScheduleId].revoked == false);
		require(vestingSchedules[vestingScheduleId].paused == false);
		_;
	}
	 ...
	 * @param _paused whether the vesting schedule is created in paused state or not
...
*/
	...
	function setPaused(
		...
		VestingSchedule storage vestingSchedule = vestingSchedules[vestingScheduleId];
		require(vestingSchedule.paused != paused, "TokenVesting: flag is already set");

		vestingSchedule.paused = paused;

		emit Paused(msg.sender, vestingSchedule.beneficiary, vestingScheduleId, paused);
	}
	...
	function release(
		bytes32 vestingScheduleId,
		uint128 amount
	) public virtual nonReentrant onlyIfVestingScheduleNotPaused(vestingScheduleId) {
		VestingSchedule storage vestingSchedule = vestingSchedules[vestingScheduleId];
