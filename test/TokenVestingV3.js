...
describe("TokenVestingV3", function () {
	let Token;
	...
	function run_test_suite(deployment_fn) {
		describe("Vesting", function() {
			...
			it("Should check input parameters for createVestingSchedule method", async function() {
				...
				await expect(tokenVesting.createVestingSchedule(addr1.address, time, 0, 0, 1, false, false, 1, 0)).to.be.revertedWith(
					"TokenVesting: duration must be > 0"
				);
				await expect(tokenVesting.createVestingSchedule(addr1.address, time, 0, 1, 0, false, false, 1, 0)).to.be.revertedWith(
					"TokenVesting: slicePeriodSeconds must be >= 1"
				);
				await expect(tokenVesting.createVestingSchedule(addr1.address, time, 0, 1, 1, false, false, 0, 0)).to.be.revertedWith(
					"TokenVesting: amount must be > 0"
				);
				await expect(tokenVesting.createVestingSchedule(addr1.address, time, 0, 1, 1, false, false, 1, 2)).to.be.revertedWith(
					"TokenVesting: immediatelyReleasableAmount must be <= amount"
				);
			});
		});
	}
	run_test_suite(deploy_v3);
});
